# 1. Importamos la clase principal del framework
from flask import Flask

# 2. INSTANCIACIÓN: Creamos el objeto servidor
# __name__ le dice a Flask dónde buscar los archivos internos del proyecto
app = Flask(__name__)

# 3. ENRUTAMIENTO
# El decorador @app.route mapea una URL con una función
@app.route('/', methods=['GET'])
def home() -> str:
    """Función que se ejecuta cuando el cliente hace GET a la raíz del sitio"""
    return "<h1>Sistema de Control Agroindustrial Activo</h1>"

# 4. EJECUCIÓN DEL SERVIDOR
if __name__ == '__main__':
    # host 0.0.0.0 permite que otros equipos de la red accedan
    # port 5000 es el puerto del servidor
    # debug=True reinicia el servidor automáticamente cuando haces cambios
    print("Iniciando servidor (http://127.0.0.1:5000)")
    app.run(host='0.0.0.0', port=5000, debug=True)